# 20260704_July4th_HeroDay_Microdart

## Campaign Info
- **Template:** `base_block_promo_sale_0520`（促销 BLOCK，深底）
- **Type:** Promotional / Peak Day
- **Send Date:** 2026-07-04 (Saturday — 正日)
- **Send Time:** 9:00 AM ET（SMS 正日 alert 11 AM 跟进）
- **Segment:** 全列表（标准）
- **Product Focus:** B — Deepcare+ Micro-dart Eye Patch（招牌爆品，up to 50%；主打 Q2 新增 retinol）
- **Goal:** 正日全站营收高峰；招牌 micro-dart 最低价做 hero（去年首封王者 offer 移到正日）。
- **Klaviyo Campaign Name:** `[DEP]_20260704_July4th_HeroDay`

---

## Subject Lines
1. Happy 4th — our micro-dart hero is up to 50% off 🎆
2. The 4th is here: up to 50% off the patch everyone wants.
3. Peak day: micro-darts up to 50% off + 40% sitewide.

## Preview Text
Our best-selling micro-dart eye patch, up to 50% off — the lowest of the event. Plus 40% sitewide with JULY4TH10.

---

## Hero Section
**Hero Image 方向：** Micro-dart eye patch tin as hero, on dark reflective navy surface with restrained gold firework bokeh; one patch shown close enough to read the dart texture; red/white/blue "UP TO 50% OFF" star badge. Premium, peak-day energy.

**Headline：**
Happy 4th. The Sale Peaks Today.

**Subheadline：**
Line 1: Micro-darts up to 50% off.
Line 2: + 40% sitewide, JULY4TH10.

**Hero CTA：**
SHOP THE 4TH
- **链接：** `https://depology.com/discount/JULY4TH10?redirect=/products/deepcare-serum-infused-micro-dart-patches-lp1-t0`

---

## Body Section
**Body Headline：**
The patch, newly upgraded.

**Body Copy（2 段）：**
Paragraph 1: Our micro-dart eye patch just got an upgrade — now pairing retinol with argireline across 1,000+ self-dissolving darts that work below the surface.

Paragraph 2: Today it's up to 50% off — the lowest price of the event. Add JULY4TH10 for an extra 10% on everything else.

---

## Product Section
**Section Title：** Peak-Day Picks

### Product 1
- **横条标签：** UP TO 50% OFF · LOWEST OF THE EVENT
- **产品名：** Deepcare+ Micro-dart Eye Patch
- **描述：** 1,000+ self-dissolving darts of retinol + argireline to soften the look of under-eye lines. Newly upgraded.
- **价格：** up to **50% OFF**
- **CTA：** SHOP NOW
- **链接：** `https://depology.com/discount/JULY4TH10?redirect=/products/deepcare-serum-infused-micro-dart-patches-lp1-t0`

### Product 2
- **横条标签：** 3 FOR 2 · BEST
- **产品名：** Peptide Complex 10% Argireline™ Serum (3 for 2)
- **描述：** Relaxes the look of expression lines before they set in. Three for the price of two.
- **价格：** **$98** ~~$147~~
- **CTA：** SHOP 3 FOR 2
- **链接：** `https://depology.com/discount/JULY4TH10?redirect=/products/offer-peptide-complex-10-serum-3-for-2`

### Product 3
- **横条标签：** $89 + FREE M3K
- **产品名：** Peptide Activation Trio
- **描述：** The 3-step priming routine, $89 — first 500 orders still include a free Matrixyl® 3000.
- **价格：** **$89**
- **CTA：** SHOP THE TRIO
- **链接：** `https://depology.com/products/peptide-activation-trio`

### Product 4
- **横条标签：** 30–40% OFF
- **产品名：** Replenishing Night Under Eye Patch
- **描述：** Overnight under-eye patches that replenish and hydrate the delicate eye area while you sleep — wake to a smoother-looking eye zone.
- **价格：** 30–40% OFF
- **CTA：** SHOP NOW
- **链接：** `https://depology.com/discount/JULY4TH10?redirect=/products/replenishing-night-under-eye-patch`

---

## Closing Section
**Closing Copy：**
The 4th only comes once a year — and so does the lowest price on our micro-darts. Shop the peak before momentum slows.

**Final CTA：**
SHOP THE PEAK

---

## Mandatory Compliance Disclaimer（底部 small text — micro-dart 含 retinol，必须）
> **A note on Retinol:** Apply sunscreen during daytime — retinol can increase sensitivity to UV. New to retinol? Your skin may need a short adjustment period (mild redness or slight peeling can be normal at first; this typically subsides within a few uses). Sensitive skin: start 2–3 times per week and increase gradually. Not recommended during pregnancy, breastfeeding, or for those with a known retinol allergy.

---

## Hero Image Brief
**概念：** A round micro-dart patch tin as the hero on a dark reflective navy surface, restrained gold firework bokeh in the background, one patch angled to catch light and show the dart texture, red/white/blue star badge motif. Peak-day premium energy, no text overlay.
**风格：** MEL Style 促销变体 — deep navy, peak energy, premium。
**AI Prompt：**
```
Hero product shot of a round skincare micro-dart patch tin on a dark reflective navy surface, one patch angled to show fine dart texture, restrained gold firework bokeh in deep navy background, subtle red white and blue star accent, premium cosmetics photography, no text overlay, 4th of July peak sale day, --ar 3:2 --style raw
```

---

## 备注
- **合规：** micro-dart 现含 retinol → disclaimer 必放；用 "soften the look of / below the surface"，不写 injectable/penetrate deep。
- **链接：** micro-dart 50% 为 sale price，JULY4TH10 叠加规则 ⚠️ 待 Leon Shopify 确认；Trio $89 走标准链接不叠。
