# 20260520_AM_Memorial_Launch

## Campaign Info
- **Template:** Memorial 主题型（白底 + 红蓝 STAR badge，模板 A — 详见 `knowledge/visual/Memorial Sale Style.md`）
- **Type:** Promotional / Sale Launch — Day 1 双发第 1 封
- **Send Date:** 2026-05-20 (Wednesday — Memorial Sale Day 1)
- **Send Time:** **9:00 AM ET**（**Smart Sending OFF** — 与 5 PM 配合双发同人群）
- **Segment:** Full List（exclude buyers <7 days + 标准 Excluded segments per CLAUDE.md）
- **Product Focus:** Sitewide 50% OFF + MEM10 subscriber code (+10%, 48hrs) — Bakuchiol 70% deal anchor 单独存在；50%+10% 叠加 ≈ 55%，Bakuchiol 实际 73%
- **Goal:** Memorial Sale 启动日最大单日 revenue（参考 2025 5/21 AM 单封 $36,905）
- **Klaviyo Campaign Name:** `[DEP]_20260520_Memorial_Launch_AM`

---

## Subject Lines（A/B 测试）

**A 组（直白型 — 复刻 2025 高 OR 套路）：**
1. Memorial Day Sale IS LIVE! 50% off + extra 10% with MEM10. 🇺🇸
2. The Summer Sale You've Been Waiting For Is Here.

**B 组（含蓄 / 价值型）：**
3. 50% off just dropped (and subscribers get 10% more).
4. Memorial weekend is here. So is 50% off + extra 10%.

## Preview Text
50% off sitewide + extra 10% with code MEM10 for subscribers. 48 hours only. Shop now before it's gone.

---

## Hero Section

**Hero Image 方向：** 4-5 款 best-seller 群像摆拍（Matrixyl 3000 双瓶 + Argireline Serum + Matriplex Cream + Bakuchiol Stick + Face & Eye Duo），白底 + 微红蓝美式布料 accent，右侧或居中叠加 STAR badge "50% OFF + 10%"（红蓝双圈 + 5 星 + 黑色大字），premium 摆拍质感不廉价。

**Headline (深蓝 #1E3A8A，居中)：**
The Summer Sale You've Been Waiting For… IS LIVE!

**Hero Badge：**
⭐ ⭐ ⭐
**50% OFF + 10%**
⭐ ⭐
（红蓝双圈 STAR badge）

---

## Lead Copy（居中，黑色 18px Century Gothic）

Memorial Day is here. We're offering our **BIGGEST** summer deal — with new launches and our best-selling peptide series.

**Extra 10% off site-wide with code MEM10 for subscribers, 48 hours only.**
（红色加粗 + MEM10 下划线）

---

## Primary CTA（黑底白字按钮）
**SHOP MEMORIAL DAY SALE**
- **链接：** `https://depology.com/discount/MEM10?redirect=/pages/memorialday-sale-2026`

---

## 4-Card Product Grid（2×2）

### Card 1 — 蓝色横条 "3 FOR 2" + BEST 角标 + Dermatologist seal
- **产品名：** Matrixyl® 3000 Collagen Serum 3 for 2
- **Tag：** Target Static Wrinkles
- **评分：** ⭐⭐⭐⭐½ (2000+)
- **价格：** **$80** ~~$120~~
- **CTA：** SAVE $40 - SHOP NOW
- **链接：** `https://depology.com/discount/MEM10?redirect=/products/offer-matrixyl-r-3000-triple-bundle` **待 Leon 提供新建 Memorial promo SKU 的 slug**

### Card 2 — 蓝色横条 "3 FOR 2" + BEST + Seal
- **产品名：** Peptide Complex Argireline™ Serum 3 for 2
- **Tag：** Target Dynamic Wrinkles
- **评分：** ⭐⭐⭐⭐½ (800+)
- **价格：** **$98** ~~$147~~
- **CTA：** SAVE $49 - SHOP NOW
- **链接：** `https://depology.com/discount/MEM10?redirect=/products/offer-peptide-complex-10-serum-3-for-2` **待 Leon 激活 Argireline Triple Bundle (id 7962315849955) draft → active**

### Card 3 — 蓝色横条 "50% OFF" + BEST + Seal + 🔴 LIMITED 100 spots
- **产品名：** Face & Eye Peptide Firming Duo
- **Tag：** Smooth & Firm — Cream Duo
- **评分：** ⭐⭐⭐⭐½ (500+)
- **价格：** **$57** ~~$114~~
- **CTA：** SAVE $57 - SHOP NOW
- **链接：** `https://depology.com/discount/MEM10?redirect=/products/face-eye-peptide-firming-duo`
- **副标 / 角标：** "🔴 ONLY 100 SPOTS"

### Card 4 — 蓝色横条 "70% OFF" — Bakuchiol $9 锚位
- **产品名：** Bakuchiol Smoothing Serum Stick
- **Tag：** Improve Fine Lines & Wrinkles
- **评分：** ⭐⭐⭐⭐½
- **价格：** **$9** ~~$30~~
- **CTA：** SAVE $21 - SHOP NOW
- **链接：** `https://depology.com/discount/MEM10?redirect=/products/bakuchiol-smoothing-serum-stick`

---

## Closing Section（黑底白字 banner）

**Closing Copy：**
With limited stock available, now's the time to secure your picks!

**Secondary CTA（白底黑字按钮）：**
**SHOP ALL SALE**
- **链接：** `https://depology.com/discount/MEM10?redirect=/pages/memorialday-sale-2026`

---

## Hero Image Brief
**AI Prompt：**
```
Premium product still life arrangement of 4-5 luxury skincare products on white surface with subtle red and navy blue ribbon accents at edges, soft natural studio lighting, products grouped in artful composition: tall serum dropper bottles, white peptide cream jar, slim white serum stick, product packaging with reflective surfaces, clean editorial layout, sophisticated cosmetics photography, no text or graphics overlay, hero shot for premium skincare brand, --ar 3:2 --style raw
```

---

## 备注

### 关键执行
- **Smart Sending 必须关闭** — 否则 5PM resend 会被屏蔽
- Sale page (`/pages/memorialday-sale-2026`) 必须在 5/19 24:00 前建好
- MEM10 在 Shopify Discounts 配置完成（Order Discount Code，48hrs，no min spend，site-wide）
- Shopify Combinations 启用 Product + Order 叠加
- Face & Eye Duo 100-cap Automatic Discount 配置完成（usage limit 100）

### 预期 Revenue
- **$16,000 - $21,000**（2025 5/21 AM $36,905 减半基准）

### 与 5 PM Resend 的关系
- 同一人群，**完全不同的 H1 / Hero / Lead / 4-card grid**（不是简单换 SL，是动量叙事重写）
- SMS Launch alert 在 11 AM 触达（夹在 AM + PM email 中间）

### 链接策略
- Day 1-2 (5/20-5/22 9 AM ET) MEM10 有效期内：所有产品链接走 `/discount/MEM10?redirect=...`
- Day 3+ MEM10 失效后：所有邮件链接切回标准产品 URL
