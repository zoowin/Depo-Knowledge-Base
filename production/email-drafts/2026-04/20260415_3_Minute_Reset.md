# 20260415_3_Minute_Reset

## Campaign Info
- **Template:** R5x7wg
- **Type:** Lifestyle / Cross-sell
- **Send Date:** 2026-04-15
- **Segment:** Engaged
- **Product Focus:** Line A — Matriplex Cream + Line B — Eye Cream + Line F — Opuntia Cleansing Balm（3步系统）
- **Goal:** 生活方式切入，展示"极简但有效"的 3 步 routine，推动 Opuntia 交叉销售

---

## Subject Lines
1. No time? This 3-step routine does the work for you.
2. 3 minutes. 3 products. Real results.
3. The busy-day routine that actually works ⏱️
4. Science-backed skincare in under 3 minutes.

## Preview Text
Three products. Three minutes. Zero compromise on results.

---

## Hero Section
> 📐 对应 Klaviyo 位置：Hero Image 下方第一个 Text Block

**Hero Image 方向：** 浴室台面上整齐排列 3 件产品，旁边有一个小计时器显示 3:00。温暖晨光，生活感但不凌乱。

**Headline：**
The 3-Minute Reset.

**Subheadline：**
Line 1: Busy mornings happen.
Line 2: Your skin shouldn't pay for it.

**Hero CTA：**
SEE THE ROUTINE

---

## Body Section
> 📐 对应 Klaviyo 位置：Hero 下方 Text Block

**Body Headline：**
Simple Beats Complicated.

**Body Copy（3 段）：**
Paragraph 1: Between meetings, school runs, and everything in between — some mornings, 10 steps just isn't happening. We get it.
Paragraph 2: The good news: effective skincare doesn't require a 20-minute ritual. Three well-chosen products can deliver more than a shelf full of average ones.
Paragraph 3: Here's the routine we'd recommend when time is short but standards aren't: cleanse, treat, seal. Done in 3 minutes.

---

## Product Section
> 📐 对应 Klaviyo 位置：Product Cards 区域

**Section Title：**
Your 3-Step Morning System

### Product 1
- **角色标签（VE92sd）：** —
- **产品名：** Opuntia-C Relief Cleansing Balm
- **描述：** Step 1: Melt away overnight buildup without stripping your barrier. Prickly Pear + Squalane cleanse and hydrate simultaneously.
- **CTA：** SHOP CLEANSING BALM
- **链接：** [Shopify 产品链接]

### Product 2
- **角色标签（VE92sd）：** —
- **产品名：** Argireline™ Eye Cream
- **描述：** Step 2: A quick dab around the eyes. Argireline™ helps smooth expression lines. Light enough for under makeup.
- **CTA：** SHOP EYE CREAM
- **链接：** [Shopify 产品链接]

### Product 3
- **角色标签（VE92sd）：** —
- **产品名：** Matriplex™ Peptide Intense Cream
- **描述：** Step 3: Lock everything in. 8% triple peptide complex hydrates, firms, and protects — all before you grab your keys.
- **CTA：** SHOP MATRIPLEX CREAM
- **链接：** [Shopify 产品链接]

---

## Closing Section
> 📐 对应 Klaviyo 位置：Product Cards 下方最后一个 Text Block

**Closing Copy：**
3 minutes today. Better skin every day. That's the deal.

**Final CTA：**
SHOP THE 3-STEP ROUTINE

---

## Hero Image Brief
> 📐 用于 ChatGPT / Midjourney 生成图片，不放入 Klaviyo

**概念：** Three products neatly arranged on a bathroom counter, beside a small timer showing 3:00. Warm morning light, lived-in but elegant.
**风格：** MEL Style — lifestyle editorial, warm but clean.
**AI Prompt：**
```
Three elegant skincare products lined up on a clean bathroom countertop beside a small white timer. Warm morning sunlight streaming through a window, soft shadows, fresh towel in background. Lifestyle beauty editorial, organized and minimal, conveying speed and simplicity. Premium brand aesthetic --ar 3:2
```
