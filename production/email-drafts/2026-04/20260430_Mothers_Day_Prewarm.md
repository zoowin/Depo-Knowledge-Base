# 20260430_Mothers_Day_Prewarm

## Campaign Info
- **Template:** R5x7wg
- **Type:** Pre-warm / List Building
- **Send Date:** 2026-04-30
- **Segment:** Full List
- **Product Focus:** Peptide Duo + Caviar Stick bundle ($109) + Anti-Aging Trio ($99) + Face & Eye Duo ($79) — 母亲节礼物定位
- **Goal:** Mother's Day 预热，建立 Early Interest 列表，为 5 月正式促销蓄力

---

## Subject Lines
1. Still searching for the perfect gift? We have an idea.
2. Mother's Day is coming — the gift she'll actually use
3. The gift that keeps working, long after she opens it 💝
4. For the woman who deserves more than flowers.

## Preview Text
Something she'll use every day. Something that actually works. Coming soon.

---

## Hero Section
> 📐 对应 Klaviyo 位置：Hero Image 下方第一个 Text Block

**Hero Image 方向：** 礼盒概念 — 优雅的包装盒微微打开，透出 Depology 产品。温暖柔和的光线，母爱/温情但不俗气。

**Headline：**
The Gift She'll Actually Use.

**Subheadline：**
Line 1: Mother's Day is May 11.
Line 2: Give her something that works.

**Hero CTA：**
GET EARLY ACCESS

---

## Body Section
> 📐 对应 Klaviyo 位置：Hero 下方 Text Block

**Body Headline：**
Beyond Flowers and Candles.

**Body Copy（3 段）：**
Paragraph 1: Every year, the same question: what do you get for the woman who has everything? This year, the answer is simple — something she'll use every single day.
Paragraph 2: Our Peptide Serum Duo combines two clinically studied formulas: one for hydration and structure, one for expression lines. Together, they deliver a complete routine she can start immediately.
Paragraph 3: We're preparing something special for Mother's Day. Sign up for early access and be the first to know when it drops — plus exclusive pricing for early birds.

---

## Product Section
> 📐 对应 Klaviyo 位置：Product Cards 区域

**Section Title：**
The Perfect Duo

### Product 1
- **角色标签（VE92sd）：** —
- **产品名：** Peptide Duo + Caviar Multi-Balm Serum Stick
- **描述：** Two peptide serums + a luxurious caviar serum stick. Three products, one elegant gift. $109 (save $10). The set she'll actually use every day.
- **CTA：** SHOP PEPTIDE + CAVIAR SET
- **链接：** https://www.depology.com/products/peptide-duo-free-caviar-multi-balm-serum-stick

### Product 2
- **角色标签（VE92sd）：** —
- **产品名：** Anti-Aging Trio
- **描述：** The complete starter kit for mom: Micro-dart Patches + Matrixyl Serum + Peptide Serum. A 4-week system in one box. $99 (save $62).
- **CTA：** SHOP THE TRIO
- **链接：** https://www.depology.com/products/anti-agingtrio

### Product 3
- **角色标签（VE92sd）：** —
- **产品名：** Face & Eye Peptide Firming Duo
- **描述：** Face cream + Eye cream in one set. Complete peptide care for the two areas that matter most. $79 (save $35).
- **CTA：** SHOP FACE & EYE DUO
- **链接：** https://www.depology.com/products/face-eye-peptide-firming-duo

---

## Closing Section
> 📐 对应 Klaviyo 位置：Product Cards 下方最后一个 Text Block

**Closing Copy：**
Mother's Day: May 11. Something special is coming. Get on the list.

**Final CTA：**
JOIN EARLY ACCESS

---

## Hero Image Brief
> 📐 用于 ChatGPT / Midjourney 生成图片，不放入 Klaviyo

**概念：** Elegant gift box slightly opened, revealing Depology products inside. Warm, soft light. Mother's Day feeling without being cliché.
**风格：** MEL Style + gift/warm tones. Cream/gold/soft pink palette.
**AI Prompt：**
```
An elegant cream-colored gift box, slightly open, revealing two skincare serums nestled inside on soft tissue paper. Warm, soft lighting suggesting morning light. A single stem of pink peony beside the box. Premium beauty gifting aesthetic, sophisticated and warm, Mother's Day mood without being overly sentimental --ar 3:2
```

---

## 备注
⚠️ **Operational:** 需提前建立 Mother's Day Early Access 列表/注册页面。CTA 链接指向 Klaviyo 表单或 Landing Page。确认 Peptide Serum Duo 礼盒是否可用（与产品团队沟通）。
