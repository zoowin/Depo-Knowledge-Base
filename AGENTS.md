# Depology EDM System — Codex Operating Manual
...
```
Depo-Knowledge-Base/
├── AGENTS.md              ← YOU ARE HERE (always read first)
...
│
└── .skills/               ← Codex SKILLS (callable workflows)
...
- **Codex 生成完整 HTML**，Leon 只需上传 hero image
...
### Phase 0: Monthly Calendar (Codex)
...
### Phase 1: Batch Email Drafting (Codex)
...
### Phase 2: Build HTML & Deploy (Codex — 本地脚本，零 token)
1. Codex 选择基础模板（白底教育型 → 0408，黑底促销型 → 0403）
2. Codex 写 Python 脚本从基础模板替换内容（文案/图片/链接/CTA）→ 本地生成完整 HTML
...
1. Leon 使用 Codex 提供的 AI Prompt 在 ChatGPT / Midjourney 生成 Hero Image
2. 在 Klaviyo campaign 中上传 Hero Image（唯一手动步骤）
3. Preview → Schedule or Send

### Phase 4: Post-Send (Codex)
